...
const iron0=rd.inventory.starIron;const rr=game.resourceDungeon(rd,'forge');assert.equal(rr.reward.starIron,65);assert.equal(rd.inventory.starIron,iron0+65);game.resourceDungeon(rd,'forge');assert.throws(()=>game.resourceDungeon(rd,'forge'),/DUNGEON_ATTEMPTS_EXHAUSTED/);
...
assert.equal(v8.version,20);assert.equal(v8.heroes.h004.awakening,0);assert.equal(game.unreadMailCount(v8),0,'new v2.1.5 accounts must not receive legacy compensation');
delete v8.systemMailPolicy;v8.mails=[];v8=game.migrate(v8,'v8');assert.equal(game.unreadMailCount(v8),2,'legacy saves retain historical compensation entitlement');
...
const old7=game.newPlayer('old7');old7.version=7;delete old7.mails;delete old7.systemMailPolicy;delete old7.eventBoss;delete old7.inventory.signatureWeapons;delete old7.inventory.awakeningCore;delete old7.heroes.h004.awakening;delete old7.heroes.h004.signature;const mig8=game.migrate(old7,'old7');assert.equal(mig8.version,20);assert.ok(Array.isArray(mig8.mails)&&mig8.mails.length>=2);assert.ok(mig8.eventBoss);assert.equal(mig8.heroes.h004.awakening,0);
...
let login=game.newPlayer('login11');const coinBefore=login.coin,baseDay=game.gameDay(),nextDay=n=>{const d=new Date(baseDay+'T00:00:00Z');d.setUTCDate(d.getUTCDate()+n);return d.toISOString().slice(0,10)};let lc=game.claimLoginReward(login);assert.equal(lc.day,1);assert.equal(login.coin,coinBefore+2000);assert.throws(()=>game.claimLoginReward(login),/LOGIN_REWARD_ALREADY_CLAIMED/);
for(let n=1;n<=6;n++){login.lastSeenDay=nextDay(n);lc=game.claimLoginReward(login);}
assert.equal(lc.day,7);assert.equal(login.loginReward.claimed.filter(Boolean).length,7);login.lastSeenDay=nextDay(7);const cycle2=game.claimLoginReward(login);assert.equal(cycle2.day,1);assert.equal(cycle2.cycle,2);assert.equal(login.loginReward.claimed[0],true);assert.equal(login.loginReward.claimed.slice(1).some(Boolean),false);
...
assert.equal(game.SAVE_VERSION,20);assert.equal(game.CONFIG_VERSION,'2026.09.v21.5');
