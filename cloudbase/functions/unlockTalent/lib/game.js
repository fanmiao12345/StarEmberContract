...
function ensureEventShop(state,date=new Date()){const week=gameWeek(date);state.event||={id:EVENT_CONFIG.id,moonSeal:0,clears:[],firstClears:[],shop:{},shopWeek:week,day:gameDay(date),runs:0};state.event.shop||={};if(!state.event.shopWeek)state.event.shopWeek=week;else if(state.event.shopWeek!==week){state.event.shopWeek=week;state.event.shop={};}return state.event;}
function ensureNightEventShop(state,date=new Date()){const week=gameWeek(date);state.nightEvent||={id:NIGHT_EVENT_CONFIG.id,voyageBadge:0,clears:[],firstClears:[],shop:{},shopWeek:week,day:gameDay(date),runs:0};state.nightEvent.shop||={};if(!state.nightEvent.shopWeek)state.nightEvent.shopWeek=week;else if(state.nightEvent.shopWeek!==week){state.nightEvent.shopWeek=week;state.nightEvent.shop={};}return state.nightEvent;}
...
function migrate(input,ownerId='guest'){
  ...
  s.inventory||={equipment:{},relics:{},signatureWeapons:{},starIron:0,forgeDust:0,bondGift:0,awakeningCore:0,enhance:{}};s.inventory.equipment=(s.inventory.equipment&&typeof s.inventory.equipment==='object')?s.inventory.equipment:{};s.inventory.relics=(s.inventory.relics&&typeof s.inventory.relics==='object')?s.inventory.relics:{};s.inventory.signatureWeapons=(s.inventory.signatureWeapons&&typeof s.inventory.signatureWeapons==='object')?s.inventory.signatureWeapons:{};s.inventory.heroFragments=(s.inventory.heroFragments&&typeof s.inventory.heroFragments==='object')?s.inventory.heroFragments:{};for(const bagName of ['equipment','relics','signatureWeapons','heroFragments'])for(const id of Object.keys(s.inventory[bagName])){const count=safeInt(s.inventory[bagName][id],0,0);if(count>0)s.inventory[bagName][id]=count;else delete s.inventory[bagName][id];}s.inventory.starIron=safeInt(s.inventory.starIron,0,0);s.inventory.forgeDust=safeInt(s.inventory.forgeDust,0,0);s.inventory.bondGift=safeInt(s.inventory.bondGift,0,0);s.inventory.awakeningCore=safeInt(s.inventory.awakeningCore,0,0);s.inventory.talentPoints=safeInt(s.inventory.talentPoints,0,0);s.inventory.enhance=(s.inventory.enhance&&typeof s.inventory.enhance==='object')?s.inventory.enhance:{};
  ...
  s.resourceRuns||={day:gameDay(),used:{}};if(s.resourceRuns.day!==gameDay())s.resourceRuns={day:gameDay(),used:{}};s.resourceRuns.used=(s.resourceRuns.used&&typeof s.resourceRuns.used==='object')?s.resourceRuns.used:{};for(const id of Object.keys(s.resourceRuns.used))s.resourceRuns.used[id]=safeInt(s.resourceRuns.used[id],0,0);
  ...
  ensureWeekly(s);ensureShop(s);s.formationPresets=(s.formationPresets&&typeof s.formationPresets==='object')?s.formationPresets:{};s.talents=(s.talents&&typeof s.talents==='object')?s.talents:{};for(const id of Object.keys(s.heroes||{}))heroTalents(s,id);const gd=gameDay();s.sweep=(s.sweep&&typeof s.sweep==='object')?s.sweep:{day:gd,used:0};if(s.sweep.day!==gd)s.sweep={day:gd,used:0};s.sweep.used=safeInt(s.sweep.used,0,0);s.nightEvent=(s.nightEvent&&s.nightEvent.id===NIGHT_EVENT_CONFIG.id)?s.nightEvent:{id:NIGHT_EVENT_CONFIG.id,voyageBadge:0,clears:[],firstClears:[],shop:{},day:gd,runs:0};s.nightEvent.voyageBadge=safeInt(s.nightEvent.voyageBadge,0,0);s.nightEvent.clears=Array.isArray(s.nightEvent.clears)?s.nightEvent.clears:[];s.nightEvent.firstClears=Array.isArray(s.nightEvent.firstClears)?s.nightEvent.firstClears:[];s.nightEvent.shop=s.nightEvent.shop||{};s.nightEvent.shopWeek=s.nightEvent.shopWeek||gameWeek();if(s.nightEvent.shopWeek!==gameWeek()){s.nightEvent.shopWeek=gameWeek();s.nightEvent.shop={};}if(s.nightEvent.day!==gd){s.nightEvent.day=gd;s.nightEvent.runs=0;}s.nightEvent.runs=safeInt(s.nightEvent.runs,0,0);
  ...
  s.formation=[...new Set(s.formation.filter(id=>s.heroes[id]))].slice(0,5);if(!s.formation.length&&s.heroes.h004)s.formation=['h004'];s.formationRows=(s.formationRows&&typeof s.formationRows==='object')?s.formationRows:{};s.formation.forEach((id,i)=>{if(!['front','back'].includes(s.formationRows[id]))s.formationRows[id]=i<2?'front':'back';});for(const id of Object.keys(s.formationRows))if(!s.formation.includes(id))delete s.formationRows[id];
...
function battle(state,rng=rand01,options={}){
  ...
  if(state.campaignCompleted)throw codeError('CAMPAIGN_COMPLETED');if(!state.formation.length)throw codeError('INVALID_FORMATION');const skillMode=options.skillMode==='manual'?'manual':'auto',manualSkills=[...new Set((options.manualSkills||[]).filter(id=>state.formation.includes(id)))].slice(0,3),autoStrategy=AUTO_STRATEGIES[options.autoStrategy]?options.autoStrategy:(state.battlePrefs?.autoStrategy||'balanced');state.battlePrefs={skillMode,manualSkills,autoStrategy};const stage=state.stage,story=storyForStage(stage);state.storySeen||=[];if(story&&!state.storySeen.includes(story.id))state.storySeen.push(story.id);const boss=bossByStage(stage),allies=state.formation.map((id,i)=>makeUnit(byId(id),state.heroes[id],state,state.formationRows?.[id]||(i<2?'front':'back')));
