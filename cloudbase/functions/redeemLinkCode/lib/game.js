...
const BOSSES = [
 ...
 {stage:25,chapter:5,name:'裂空古兽',faction:'青原',skill:'裂空咆哮',skillType:'bossAoe',hp:35000,atk:1350,def:480,spd:104,firstReward:{equipment:'eq007'}},
 {stage:30,chapter:6,name:'灰烬王座',faction:'灰境',skill:'终焉星坠',skillType:'bossFinal',hp:43000,atk:1500,def:600,spd:118,firstReward:{relic:'r004'}},
];
...
function coreTalentFor(h){
 if(h.id==='h003')return{desc:'镜潮治疗回响 +12%',bonus:{healPct:.12}};
 if(h.id==='h013')return{desc:'引烬增幅：攻击 +8%，生命 +8%',bonus:{atkPct:.08,hpPct:.08}};
 if(h.role==='防御')return{desc:'防御 +10%，护盾 +10%',bonus:{defPct:.10,shieldPct:.10}};
 if(h.role==='治疗')return{desc:'治疗 +14%',bonus:{healPct:.14}};
 if(h.id==='h005')return{desc:'引磁域眩晕命中 +12%',bonus:{effectPct:.12}};if(h.id==='h012')return{desc:'霜痕沉默有12%概率额外持续1回合',bonus:{effectPct:.12}};if(h.role==='控制')return{desc:'控制效果 +12%',bonus:{effectPct:.12}};
 if(h.role==='刺击')return{desc:'斩杀伤害 +14%',bonus:{skillPct:.14}};
 if(h.role==='术式')return{desc:'技能伤害 +10%',bonus:{skillPct:.10}};
 return{desc:'技能伤害 +12%',bonus:{skillPct:.12}};
}
const TALENT_TREES=Object.fromEntries(HEROES.map(h=>{const core=coreTalentFor(h);return[h.id,[
 ...
 {id:'core',name:`${h.skill}·本命`,cost:3,requires:'edge',desc:core.desc,bonus:core.bonus}
]];}));
...
const NIGHT_EVENT_STAGES=[
 ...
 {id:'N5',name:'夜航终点',power:9800,reward:{voyageBadge:160,forgeDust:100},first:{talentPoints:2,tickets:2,signatureWeapon:'sig_h001'}},
];
...
const RESOURCE_DUNGEONS = [
 {id:'coin',name:'烬币矿脉',desc:'稳定产出角色升级所需烬币；直接刷取效率高于星髓兑换。',attempts:2,reward:{coin:3300},power:1200,icon:'◈'},
 {id:'forge',name:'星铁熔炉',desc:'产出装备强化材料「星铁」；专属刷取效率高于星髓兑换。',attempts:2,reward:{starIron:65},power:1800,icon:'◆'},
...
const ACHIEVEMENTS = [
 ...
 {id:'stage20',name:'重返曜庭',desc:'主线推进至第 20 关',type:'stage',goal:20,reward:{tickets:5,signatureWeapon:'sig_h002'}},
 ...
 {id:'power8000',name:'星烬成军',desc:'阵容战力达到 8000',type:'power',goal:8000,reward:{tickets:4,signatureWeapon:'sig_h003'}},
 {id:'forge1',name:'第一炉星火',desc:'完成 1 次装备锻造',type:'craftedCount',goal:1,reward:{starCrystal:120}},
 {id:'bond3',name:'星语相知',desc:'任意契灵好感达到 3 级',type:'maxBondLevel',goal:3,reward:{tickets:2,signatureWeapon:'sig_h004'}},
...
const CONFIG_VERSION = '2026.09.v21.5';
...
const DAILY_TASKS = [
 ...
 {id:'upgrade1',name:'契灵成长',desc:'完成1次培养、整备、天赋、觉醒或羁绊提升',goal:1,reward:{coin:600}},
];
...
const EVENT_STAGES = [
 ...
 {id:'E6',name:'终末月环',power:8200,reward:{moonSeal:180,starCrystal:90},first:{tickets:3,forgeDust:120,signatureWeapon:'sig_h008'}},
];
const EVENT_SHOP = [
...
const SYSTEM_MAILS = [
 {id:'v08_launch',title:'契印武装开放纪念',body:'契印武装与觉醒系统已开放。为既有引星者补发一份整备物资。',reward:{tickets:5,starCrystal:300,awakeningCore:3,signatureWeapon:'sig_h004'}},
 {id:'v08_event',title:'月蚀讨伐前线补给',body:'月蚀兽·涅槃已出现在活动深处，请收下前线整备物资。',reward:{coin:6000,starIron:120,forgeDust:180,bondGift:2}},
];
const EVENT_BOSS = {id:'eclipsePhoenix',name:'月蚀兽·涅槃',dailyAttempts:3,recommendedPower:9000,maxHp:3000000,subtitle:'全服伤害榜 · 单次伤害计入个人最佳与排行榜'};
...
function safeInt(value,fallback=0,min=0,max=Number.MAX_SAFE_INTEGER){const n=Number(value),f=Number(fallback);const base=Number.isFinite(n)?Math.trunc(n):(Number.isFinite(f)?Math.trunc(f):0);return Math.max(min,Math.min(max,base));}
...
function observedDay(state){const now=gameDay();const last=typeof state?.lastSeenDay==='string'?state.lastSeenDay:'';const effective=last&&last>now?last:now;if(state)state.lastSeenDay=effective;return effective;}
function observedWeek(state){const day=observedDay(state),d=new Date(day+'T00:00:00Z');return gameWeek(d);}
...
function newPlayer(ownerId='guest'){
  ...
  return {
    ...
    formation:['h004'], formationRows:{h004:'back'}, gachaHistory:[], inventory:{equipment:{eq001:1},relics:{},signatureWeapons:{},heroFragments:{},starIron:120,forgeDust:0,bondGift:3,awakeningCore:0,talentPoints:3,enhance:{eq001:0}}, activeRelic:null,
    ...
    event:{id:EVENT_CONFIG.id,moonSeal:0,clears:[],firstClears:[],shop:{},shopWeek:gameWeek(),day:gameDay(),runs:0}, eventBoss:{day:gameDay(),runs:0,bestDamage:0,totalDamage:0,lastDamage:0}, bond:{h004:{points:0}}, mails:[], systemMailPolicy:'current',
    profile:{displayName:'引星者',linked:false}, newbieGacha:{claimed:false}, storySeen:[], battlePrefs:{skillMode:'auto',manualSkills:[],autoStrategy:'balanced'}, formationPresets:{},talents:{},sweep:{day:gameDay(),used:0},nightEvent:{id:NIGHT_EVENT_CONFIG.id,voyageBadge:0,clears:[],firstClears:[],shop:{},shopWeek:gameWeek(),day:gameDay(),runs:0}, activeBattleSession:null, battleReplays:[],
    rewardBackfills:{v003:true}, lastSeenDay:gameDay(), lastAction:null, createdAt:created, updatedAt:created, lastLogin:created
  };
}
function ensureDaily(state){
  const day=observedDay(state);
...
function ensureWeekly(state){const week=observedWeek(state);if(!state.weekly||state.weekly.week!==week)state.weekly=emptyWeekly(week);for(const t of WEEKLY_TASKS){state.weekly.progress[t.id]??=0;state.weekly.claimed[t.id]??=false;}state.weekly.allClaimed=!!state.weekly.allClaimed;return state.weekly;}
function ensureShop(state){const day=observedDay(state);if(!state.shop||state.shop.day!==day)state.shop={day,purchases:{}};state.shop.purchases||={};return state.shop;}
function ensureEventShop(state){const week=observedWeek(state);state.event||={id:EVENT_CONFIG.id,moonSeal:0,clears:[],firstClears:[],shop:{},shopWeek:week,day:observedDay(state),runs:0};state.event.shop||={};if(!state.event.shopWeek)state.event.shopWeek=week;else if(state.event.shopWeek!==week){state.event.shopWeek=week;state.event.shop={};}return state.event;}
function ensureNightEventShop(state){const week=observedWeek(state);state.nightEvent||={id:NIGHT_EVENT_CONFIG.id,voyageBadge:0,clears:[],firstClears:[],shop:{},shopWeek:week,day:observedDay(state),runs:0};state.nightEvent.shop||={};if(!state.nightEvent.shopWeek)state.nightEvent.shopWeek=week;else if(state.nightEvent.shopWeek!==week){state.nightEvent.shopWeek=week;state.nightEvent.shop={};}return state.nightEvent;}
function recordCombatProgress(state,{battles=1,wins=0,resource=0}={}){if(battles>0){dailyProgress(state,'battle3',battles);weeklyProgress(state,'battle12',battles);}if(wins>0){dailyProgress(state,'win2',wins);weeklyProgress(state,'win8',wins);}if(resource>0)weeklyProgress(state,'resource5',resource);}
function migrate(input,ownerId='guest'){
  ...
  s.level=safeInt(s.level,1,1);s.exp=safeInt(s.exp,0,0);s.coin=safeInt(s.coin,5000,0);s.starCrystal=safeInt(s.starCrystal,300,0);s.tickets=safeInt(s.tickets,30,0);s.pity=safeInt(s.pity,0,0);s.stage=safeInt(s.stage,1,1);s.revision=safeInt(s.revision,0,0);
  s.lastSeenDay=typeof s.lastSeenDay==='string'?s.lastSeenDay:gameDay();s.heroes=(s.heroes&&typeof s.heroes==='object'&&!Array.isArray(s.heroes))?s.heroes:{};s.formation=Array.isArray(s.formation)?s.formation:[];s.gachaHistory=Array.isArray(s.gachaHistory)?s.gachaHistory:[];s.createdAt||=nowIso();s.updatedAt||=nowIso();s.lastLogin||=nowIso();
  s.profile||={displayName:'引星者',linked:false};s.profile.displayName||='引星者';s.profile.linked=!!s.profile.linked;s.newbieGacha||={claimed:false};s.newbieGacha.claimed=!!s.newbieGacha.claimed;s.storySeen=Array.isArray(s.storySeen)?[...new Set(s.storySeen.map(String))]:[];s.battlePrefs||={skillMode:'auto',manualSkills:[],autoStrategy:'balanced'};s.battlePrefs.skillMode=s.battlePrefs.skillMode==='manual'?'manual':'auto';s.battlePrefs.manualSkills=Array.isArray(s.battlePrefs.manualSkills)?s.battlePrefs.manualSkills.filter(id=>s.heroes?.[id]).slice(0,3):[];s.battlePrefs.autoStrategy=AUTO_STRATEGIES[s.battlePrefs.autoStrategy]?s.battlePrefs.autoStrategy:'balanced';s.battleReplays=Array.isArray(s.battleReplays)?s.battleReplays.slice(0,5):[];s.activeBattleSession=(s.activeBattleSession&&typeof s.activeBattleSession==='object'&&!Array.isArray(s.activeBattleSession))?s.activeBattleSession:null;if(s.activeBattleSession&&s.activeBattleSession.stage!==s.stage&&s.activeBattleSession.status!=='complete')s.activeBattleSession=null;if(s.activeBattleSession){s.activeBattleSession.stats=(s.activeBattleSession.stats&&typeof s.activeBattleSession.stats==='object')?s.activeBattleSession.stats:{};s.activeBattleSession.skillCasts=(s.activeBattleSession.skillCasts&&typeof s.activeBattleSession.skillCasts==='object')?s.activeBattleSession.skillCasts:{};s.activeBattleSession.combosUsed=Array.isArray(s.activeBattleSession.combosUsed)?s.activeBattleSession.combosUsed:[];s.activeBattleSession.allies=Array.isArray(s.activeBattleSession.allies)?s.activeBattleSession.allies.filter(u=>u&&typeof u==='object'):[];s.activeBattleSession.enemies=Array.isArray(s.activeBattleSession.enemies)?s.activeBattleSession.enemies.filter(u=>u&&typeof u==='object'):[];if(!s.activeBattleSession.allies.length||!s.activeBattleSession.enemies.length){s.activeBattleSession=null;}else for(const team of [s.activeBattleSession.allies,s.activeBattleSession.enemies])for(const u of team){u.silence=Math.max(0,Number(u.silence)||0);u.taunt=Math.max(0,Number(u.taunt)||0);u.row=['front','back'].includes(u.row)?u.row:'front';u.range=u.range||'any';u.skillCost=Math.max(1,Number(u.skillCost)||100);u.energyGain=Math.max(1,Number(u.energyGain)||34);u.skillCooldown=Math.max(0,Number(u.skillCooldown)||0);u.cooldownLeft=Math.max(0,Number(u.cooldownLeft)||0);u.combatClass=u.combatClass||'support';u.breakWindow=Math.max(0,Number(u.breakWindow)||0);if(u.isBoss){u.breakMax=Math.max(1,Number(u.breakMax)||BOSS_BREAK_RULES[s.activeBattleSession.stage]||150);u.breakGauge=Math.max(0,Math.min(u.breakMax,Number(u.breakGauge)||u.breakMax));u.breakCount=Math.max(0,Number(u.breakCount)||0);}}}
  s.inventory=(s.inventory&&typeof s.inventory==='object'&&!Array.isArray(s.inventory))?s.inventory:{equipment:{},relics:{},signatureWeapons:{},heroFragments:{},starIron:0,forgeDust:0,bondGift:0,awakeningCore:0,talentPoints:0,enhance:{}};s.inventory.equipment=(s.inventory.equipment&&typeof s.inventory.equipment==='object')?s.inventory.equipment:{};s.inventory.relics=(s.inventory.relics&&typeof s.inventory.relics==='object')?s.inventory.relics:{};s.inventory.signatureWeapons=(s.inventory.signatureWeapons&&typeof s.inventory.signatureWeapons==='object')?s.inventory.signatureWeapons:{};s.inventory.heroFragments=(s.inventory.heroFragments&&typeof s.inventory.heroFragments==='object')?s.inventory.heroFragments:{};for(const bagName of ['equipment','relics','signatureWeapons','heroFragments'])for(const id of Object.keys(s.inventory[bagName])){const count=safeInt(s.inventory[bagName][id],0,0);if(count>0)s.inventory[bagName][id]=count;else delete s.inventory[bagName][id];}s.inventory.starIron=safeInt(s.inventory.starIron,0,0);s.inventory.forgeDust=safeInt(s.inventory.forgeDust,0,0);s.inventory.bondGift=safeInt(s.inventory.bondGift,0,0);s.inventory.awakeningCore=safeInt(s.inventory.awakeningCore,0,0);s.inventory.talentPoints=safeInt(s.inventory.talentPoints,0,0);s.inventory.enhance=(s.inventory.enhance&&typeof s.inventory.enhance==='object')?s.inventory.enhance:{};
  for(const item of EQUIPMENT)s.inventory.enhance[item.id]=Math.max(0,Math.min(10,Number(s.inventory.enhance[item.id])||0));for(const [id,count] of Object.entries(s.inventory.relics)){const item=relicById(id);if(!item){delete s.inventory.relics[id];continue;}if(count>1){const extra=count-1;s.inventory.relics[id]=1;s.inventory.forgeDust+=(DISMANTLE_DUST[item.rarity]||25)*extra;}}for(const [id,count] of Object.entries(s.inventory.signatureWeapons)){const item=signatureById(id);if(!item){delete s.inventory.signatureWeapons[id];continue;}if(count>1){const extra=count-1;s.inventory.signatureWeapons[id]=1;s.inventory.forgeDust+=(item.rarity==='SSR'?300:item.rarity==='SR'?180:100)*extra;}}
  const effectiveDay=observedDay(s),effectiveWeek=observedWeek(s);s.resourceRuns=(s.resourceRuns&&typeof s.resourceRuns==='object')?s.resourceRuns:{day:effectiveDay,used:{}};if(s.resourceRuns.day!==effectiveDay)s.resourceRuns={day:effectiveDay,used:{}};s.resourceRuns.used=(s.resourceRuns.used&&typeof s.resourceRuns.used==='object')?s.resourceRuns.used:{};for(const id of Object.keys(s.resourceRuns.used))s.resourceRuns.used[id]=safeInt(s.resourceRuns.used[id],0,0);
  ...
  s.forge||={crafted:0,dismantled:0};s.forge.crafted=safeInt(s.forge.crafted,0,0);s.forge.dismantled=safeInt(s.forge.dismantled,0,0);
  s.event||={id:EVENT_CONFIG.id,moonSeal:0,clears:[],firstClears:[],shop:{},day:gameDay(),runs:0};if(s.event.id!==EVENT_CONFIG.id)s.event={id:EVENT_CONFIG.id,moonSeal:0,clears:[],firstClears:[],shop:{},day:gameDay(),runs:0};s.event.moonSeal=safeInt(s.event.moonSeal,0,0);s.event.clears=Array.isArray(s.event.clears)?s.event.clears:[];s.event.firstClears=Array.isArray(s.event.firstClears)?s.event.firstClears:[];s.event.shop||={};s.event.shopWeek=s.event.shopWeek||effectiveWeek;if(s.event.shopWeek!==effectiveWeek){s.event.shopWeek=effectiveWeek;s.event.shop={};}if(s.event.day!==effectiveDay){s.event.day=effectiveDay;s.event.runs=0;}s.event.runs=safeInt(s.event.runs,0,0);
  ensureEventBoss(s);if(s.systemMailPolicy==='current')s.mails=Array.isArray(s.mails)?s.mails.map(x=>({...x})):[];else{s.mails=mergeSystemMails(s.mails);s.systemMailPolicy='current';}
  s.bond||={};
  s.loginReward||={cycle:1,claimed:Array(7).fill(false),lastClaimDay:null,totalClaims:0};
  s.loginReward.cycle=safeInt(s.loginReward.cycle,1,1);
  ...
  s.loginReward.totalClaims=safeInt(s.loginReward.totalClaims,s.loginReward.claimed.filter(Boolean).length,0);
  ensureWeekly(s);ensureShop(s);s.formationPresets=(s.formationPresets&&typeof s.formationPresets==='object')?s.formationPresets:{};s.talents=(s.talents&&typeof s.talents==='object')?s.talents:{};for(const id of Object.keys(s.heroes||{}))heroTalents(s,id);const gd=effectiveDay;s.sweep=(s.sweep&&typeof s.sweep==='object')?s.sweep:{day:gd,used:0};if(s.sweep.day!==gd)s.sweep={day:gd,used:0};s.sweep.used=safeInt(s.sweep.used,0,0);s.nightEvent=(s.nightEvent&&s.nightEvent.id===NIGHT_EVENT_CONFIG.id)?s.nightEvent:{id:NIGHT_EVENT_CONFIG.id,voyageBadge:0,clears:[],firstClears:[],shop:{},day:gd,runs:0};s.nightEvent.voyageBadge=safeInt(s.nightEvent.voyageBadge,0,0);s.nightEvent.clears=Array.isArray(s.nightEvent.clears)?s.nightEvent.clears:[];s.nightEvent.firstClears=Array.isArray(s.nightEvent.firstClears)?s.nightEvent.firstClears:[];s.nightEvent.shop=s.nightEvent.shop||{};s.nightEvent.shopWeek=s.nightEvent.shopWeek||effectiveWeek;if(s.nightEvent.shopWeek!==effectiveWeek){s.nightEvent.shopWeek=effectiveWeek;s.nightEvent.shop={};}if(s.nightEvent.day!==gd){s.nightEvent.day=gd;s.nightEvent.runs=0;}s.nightEvent.runs=safeInt(s.nightEvent.runs,0,0);
  s.rewardBackfills=(s.rewardBackfills&&typeof s.rewardBackfills==='object')?s.rewardBackfills:{};
  if(!s.rewardBackfills.v215){
    const backfill=[];
    if(s.achievements?.claimed?.stage20)backfill.push('sig_h002');
    if(s.achievements?.claimed?.power8000)backfill.push('sig_h003');
    if(s.achievements?.claimed?.bond3)backfill.push('sig_h004');
    if(s.event?.firstClears?.includes('E6'))backfill.push('sig_h008');
    if(s.nightEvent?.firstClears?.includes('N5'))backfill.push('sig_h001');
    for(const id of [...new Set(backfill)])if((Number(s.inventory.signatureWeapons[id])||0)<1)s.inventory.signatureWeapons[id]=1;
    s.rewardBackfills.v215=true;
  }
  s.activeRelic = s.activeRelic && relicById(s.activeRelic) && safeInt(s.inventory.relics[s.activeRelic],0,0)>0 ? s.activeRelic : null;
  ...
  if(!Object.keys(s.heroes).some(id=>byId(id))){s.heroes.h004={level:1,star:1,fragments:0,awakening:0,signature:null,equipment:blankEquipment()};if((Number(s.inventory.equipment.eq001)||0)>0)s.heroes.h004.equipment.weapon='eq001';}
  for(const [id,raw] of Object.entries(s.heroes)){
    const h=byId(id);if(!h){delete s.heroes[id];continue;}const p=(raw&&typeof raw==='object'&&!Array.isArray(raw))?raw:{};s.heroes[id]=p;
    p.level=safeInt(p.level,1,1,50);p.star=safeInt(p.star,1,1,5);p.awakening=safeInt(p.awakening,0,0,3);p.signature=(p.signature&&signatureById(p.signature)?.heroId===id&&safeInt(s.inventory.signatureWeapons[p.signature],0,0)>0)?p.signature:null;
    if(p.fragments==null){const copies=safeInt(p.copies,1,1);p.fragments=Math.max(0,copies-1)*FRAGMENTS_PER_DUP[h.rarity];}else p.fragments=safeInt(p.fragments,0,0);const banked=safeInt(s.inventory.heroFragments[id],0,0);if(banked){p.fragments+=banked;delete s.inventory.heroFragments[id];}
    ...
    s.bond[id]||={points:0};s.bond[id].points=Math.min(BOND_THRESHOLDS[BOND_THRESHOLDS.length-1],Math.max(0,Number(s.bond[id].points)||0));
  }
  // v0.3 legacy backfill is one-shot. Never infer entitlement from an empty bag on every load.
  s.rewardBackfills=(s.rewardBackfills&&typeof s.rewardBackfills==='object')?s.rewardBackfills:{};
  if(!s.rewardBackfills.v003){if(!Object.keys(s.inventory.equipment).length){s.inventory.equipment.eq001=1;if(s.heroes.h004){s.heroes.h004.equipment=(s.heroes.h004.equipment&&typeof s.heroes.h004.equipment==='object')?s.heroes.h004.equipment:blankEquipment();if(!s.heroes.h004.equipment.weapon)s.heroes.h004.equipment.weapon='eq001';}}s.rewardBackfills.v003=true;}
  const equippedUse={};for(const p of Object.values(s.heroes)){for(const slot of ['weapon','armor','charm']){const itemId=p.equipment?.[slot];if(!itemId)continue;const owned=safeInt(s.inventory.equipment[itemId],0,0),used=equippedUse[itemId]||0;if(owned<=0||used>=1)p.equipment[slot]=null;else equippedUse[itemId]=used+1;}}
  s.formation=[...new Set((Array.isArray(s.formation)?s.formation:[]).filter(id=>s.heroes[id]))].slice(0,5);if(!s.formation.length)s.formation=[Object.keys(s.heroes).find(id=>byId(id))||'h004'];if(!s.formation.length&&s.heroes.h004)s.formation=['h004'];s.formationRows=(s.formationRows&&typeof s.formationRows==='object')?s.formationRows:{};s.formation.forEach((id,i)=>{if(!['front','back'].includes(s.formationRows[id]))s.formationRows[id]=i<2?'front':'back';});for(const id of Object.keys(s.formationRows))if(!s.formation.includes(id))delete s.formationRows[id];
...
function rewardState(state,reward){state.coin=safeInt(state.coin,0,0)+(Number(reward.coin)||0);state.tickets=safeInt(state.tickets,0,0)+(Number(reward.tickets)||0);state.starCrystal=safeInt(state.starCrystal,0,0)+(Number(reward.starCrystal)||0);if(reward.starIron)state.inventory.starIron=(Number(state.inventory.starIron)||0)+Number(reward.starIron);if(reward.forgeDust)state.inventory.forgeDust=(Number(state.inventory.forgeDust)||0)+Number(reward.forgeDust);if(reward.bondGift)state.inventory.bondGift=(Number(state.inventory.bondGift)||0)+Number(reward.bondGift);if(reward.awakeningCore)state.inventory.awakeningCore=(Number(state.inventory.awakeningCore)||0)+Number(reward.awakeningCore);if(reward.talentPoints)state.inventory.talentPoints=(Number(state.inventory.talentPoints)||0)+Number(reward.talentPoints);if(reward.voyageBadge){state.nightEvent||={id:NIGHT_EVENT_CONFIG.id,voyageBadge:0,clears:[],firstClears:[],shop:{},day:gameDay(),runs:0};state.nightEvent.voyageBadge=(Number(state.nightEvent.voyageBadge)||0)+Number(reward.voyageBadge);}if(reward.signatureWeapon){state.inventory.signatureWeapons||={};const id=reward.signatureWeapon,item=signatureById(id),owned=Number(state.inventory.signatureWeapons[id])||0;if(owned>=1){state.inventory.signatureWeapons[id]=1;state.inventory.forgeDust=(Number(state.inventory.forgeDust)||0)+(item?.rarity==='SSR'?300:item?.rarity==='SR'?180:100);}else state.inventory.signatureWeapons[id]=1;}if(reward.moonSeal){state.event||={id:EVENT_CONFIG.id,moonSeal:0,clears:[],firstClears:[],shop:{}};state.event.moonSeal=(Number(state.event.moonSeal)||0)+Number(reward.moonSeal);}}
...
function loginRewardInfo(state){
  const today=observedDay(state);const lr=state.loginReward||(state.loginReward={cycle:1,claimed:Array(7).fill(false),lastClaimDay:null,totalClaims:0});
...
function claimLoginReward(state){
  const today=observedDay(state);const info=loginRewardInfo(state);const lr=state.loginReward;
...
function pendingHeroFragments(state,heroId){state.inventory||={};state.inventory.heroFragments||={};return Math.max(0,Number(state.inventory.heroFragments[heroId])||0);}
function grantHeroFragments(state,heroId,amount){amount=Math.max(0,Math.floor(Number(amount)||0));if(!byId(heroId)||!amount)return{heroId,fragments:0,pending:false};const p=state.heroes[heroId];if(p){p.fragments=(Number(p.fragments)||0)+amount;return{heroId,fragments:amount,pending:false,total:p.fragments};}state.inventory.heroFragments||={};state.inventory.heroFragments[heroId]=pendingHeroFragments(state,heroId)+amount;return{heroId,fragments:amount,pending:true,total:state.inventory.heroFragments[heroId]};}
function acquireHero(state,hero){const pending=pendingHeroFragments(state,hero.id);state.heroes[hero.id]={level:1,star:1,fragments:pending,awakening:0,signature:null,equipment:blankEquipment()};if(pending)delete state.inventory.heroFragments[hero.id];state.bond||={};state.bond[hero.id]||={points:0};return pending;}
...
function rollOne(state,rng=rand01){
  ...
  if(!p)acquireHero(state,hero);else{duplicate=true;fragmentsGained=FRAGMENTS_PER_DUP[hero.rarity];p.fragments+=fragmentsGained;}
...
function forcedStandardSSR(state,rng=rand01){if(state.tickets<1)throw codeError('NOT_ENOUGH_TICKETS');state.tickets--;state.pity=0;const hero=randomPick(HEROES.filter(h=>h.rarity==='SSR'),rng),p=state.heroes[hero.id];let duplicate=false,fragmentsGained=0;if(!p)acquireHero(state,hero);else{duplicate=true;fragmentsGained=FRAGMENTS_PER_DUP.SSR;p.fragments+=fragmentsGained;}if(state.formation.length<5&&!state.formation.includes(hero.id))state.formation.push(hero.id);state.gachaHistory.unshift({heroId:hero.id,rarity:'SSR',duplicate,fragmentsGained,pool:'newbie',guaranteed:true,at:nowIso()});state.gachaHistory=state.gachaHistory.slice(0,100);return{hero,duplicate,fragmentsGained,newbieGuaranteed:true};}
...
function rollLimitedOne(state,rng=rand01){
  ...
  const p=state.heroes[hero.id];let duplicate=false,fragmentsGained=0;if(!p)acquireHero(state,hero);else{duplicate=true;fragmentsGained=FRAGMENTS_PER_DUP[hero.rarity];p.fragments+=fragmentsGained;}
...
function sweepInfo(state){const d=observedDay(state);if(!state.sweep||state.sweep.day!==d)state.sweep={day:d,used:0};return{day:d,used:state.sweep.used||0,left:Math.max(0,SWEEP_DAILY_LIMIT-(state.sweep.used||0)),limit:SWEEP_DAILY_LIMIT};}
function sweepStage(state,stage,count=1,rng=rand01){stage=Math.floor(Number(stage)||0);count=Math.max(1,Math.min(5,Math.floor(Number(count)||1)));const cleared=state.campaignCompleted||stage<state.stage;if(stage<1||stage>30||!cleared)throw codeError('STAGE_NOT_CLEARED');const info=sweepInfo(state);if(info.left<count)throw codeError('SWEEP_LIMIT');let coin=0,starIron=0,talentPoints=0,drops=[];for(let i=0;i<count;i++){coin+=180+stage*25;starIron+=stage>=10?8:4;if(rng()<.12)talentPoints++;if(stage>=20&&rng()<.045){const rarity=stage>=30&&rng()<.22?'SSR':'SR',pool=RELICS.filter(r=>r.rarity===rarity);if(pool.length){const item=randomPick(pool,rng);drops.push(addLoot(state,'relic',item.id));}}if(rng()<.16){let rarity=stage>=20?'SR':'R';if(stage>=25&&rng()<.08)rarity='SSR';const pool=EQUIPMENT.filter(e=>e.rarity===rarity);if(pool.length){const item=randomPick(pool,rng);drops.push(addLoot(state,'equipment',item.id));}}}state.coin+=coin;state.inventory.starIron=(state.inventory.starIron||0)+starIron;state.inventory.talentPoints=(state.inventory.talentPoints||0)+talentPoints;state.sweep.used+=count;recordCombatProgress(state,{battles:count,wins:count});state.updatedAt=nowIso();return{stage,count,reward:{coin,starIron,talentPoints},drops,info:sweepInfo(state)};}
function nightEventStageById(id){return NIGHT_EVENT_STAGES.find(x=>x.id===id);}
function nightEventBattle(state,id){const st=nightEventStageById(id);if(!st)throw codeError('NIGHT_EVENT_STAGE_NOT_FOUND');if(formationPower(state)<st.power)throw codeError('POWER_TOO_LOW');const d=observedDay(state);state.nightEvent||={id:NIGHT_EVENT_CONFIG.id,voyageBadge:0,clears:[],firstClears:[],shop:{},day:d,runs:0};if(state.nightEvent.day!==d){state.nightEvent.day=d;state.nightEvent.runs=0;}if(state.nightEvent.runs>=6)throw codeError('NIGHT_EVENT_ATTEMPTS_EXHAUSTED');state.nightEvent.runs++;rewardState(state,st.reward);const first=!state.nightEvent.firstClears.includes(id);if(first){state.nightEvent.firstClears.push(id);rewardState(state,st.first||{});}if(!state.nightEvent.clears.includes(id))state.nightEvent.clears.push(id);recordCombatProgress(state,{battles:1,wins:1});state.updatedAt=nowIso();return{id,name:st.name,reward:st.reward,firstReward:first?st.first:null,first};}
function nightEventShopBuy(state,id){const item=NIGHT_EVENT_SHOP.find(x=>x.id===id);if(!item)throw codeError('NIGHT_EVENT_SHOP_ITEM_NOT_FOUND');ensureNightEventShop(state);const bought=Number(state.nightEvent.shop?.[id])||0;if(bought>=item.limit)throw codeError('NIGHT_EVENT_SHOP_LIMIT');if(state.nightEvent.voyageBadge<item.cost)throw codeError('NOT_ENOUGH_VOYAGE_BADGE');state.nightEvent.voyageBadge-=item.cost;state.nightEvent.shop[id]=bought+1;const fragmentResult=item.reward.heroId?grantHeroFragments(state,item.reward.heroId,item.reward.fragments||0):null;if(!fragmentResult)rewardState(state,item.reward);state.updatedAt=nowIso();return{id,reward:item.reward,bought:bought+1,left:item.limit-bought-1,fragmentBanked:!!fragmentResult?.pending};}
...
function equipGear(state,heroId,itemId){const p=state.heroes[heroId],item=equipmentById(itemId);if(!p)throw codeError('HERO_NOT_OWNED');if(!item)throw codeError('ITEM_NOT_FOUND');const owned=Number(state.inventory?.equipment?.[itemId])||0;if(owned<=0)throw codeError('ITEM_NOT_OWNED');p.equipment||=blankEquipment();if(p.equipment[item.slot]===itemId)return{heroId,itemId,slot:item.slot,unchanged:true};if(countEquipped(state,itemId,heroId)>=1)throw codeError('ITEM_IN_USE');p.equipment[item.slot]=itemId;dailyProgress(state,'upgrade1',1);state.updatedAt=nowIso();return{heroId,itemId,slot:item.slot};}
...
function equipRelic(state,relicId){if(relicId==null||relicId===''){state.activeRelic=null;state.updatedAt=nowIso();return{relicId:null};}const relic=relicById(relicId);if(!relic)throw codeError('ITEM_NOT_FOUND');if((Number(state.inventory?.relics?.[relicId])||0)<=0)throw codeError('ITEM_NOT_OWNED');if(state.activeRelic===relicId)return{relicId,unchanged:true};state.activeRelic=relicId;dailyProgress(state,'upgrade1',1);state.updatedAt=nowIso();return{relicId};}
function addLoot(state,kind,id,count=1){count=Math.max(1,Math.floor(Number(count)||1));const item=kind==='relic'?relicById(id):equipmentById(id),bag=kind==='relic'?state.inventory.relics:state.inventory.equipment;if(kind==='relic'){const owned=Math.max(0,Number(bag[id])||0),kept=owned>0?0:1,converted=count-kept;if(kept)bag[id]=owned+kept;let dust=0;if(converted>0){dust=(DISMANTLE_DUST[item?.rarity]||25)*converted;state.inventory.forgeDust=(Number(state.inventory.forgeDust)||0)+dust;}return{kind,id,count:kept,rolledCount:count,item,duplicate:converted>0,converted:converted?{forgeDust:dust,count:converted}:null};}bag[id]=(Number(bag[id])||0)+count;return{kind,id,count,item};}
...
function giftHero(state,heroId,count=1){count=Math.max(1,Math.floor(Number(count)||1));if(!state.heroes[heroId])throw codeError('HERO_NOT_OWNED');const before=bondInfo(state,heroId);if(before.level>=5)throw codeError('BOND_CAP');const maxPoints=BOND_THRESHOLDS[BOND_THRESHOLDS.length-1],needed=Math.max(1,Math.ceil((maxPoints-before.points)/50)),used=Math.min(count,needed);if((Number(state.inventory.bondGift)||0)<used)throw codeError('NOT_ENOUGH_BOND_GIFT');state.inventory.bondGift-=used;state.bond||={};state.bond[heroId]||={points:0};state.bond[heroId].points=Math.min(maxPoints,(Number(state.bond[heroId].points)||0)+used*50);dailyProgress(state,'upgrade1',1);state.updatedAt=nowIso();return{count:used,requested:count,info:bondInfo(state,heroId)};}
function eventStageById(id){return EVENT_STAGES.find(x=>x.id===id);}
function eventBattle(state,id){const st=eventStageById(id);if(!st)throw codeError('EVENT_STAGE_NOT_FOUND');if(formationPower(state)<st.power)throw codeError('POWER_TOO_LOW');const d=observedDay(state);state.event||={id:EVENT_CONFIG.id,moonSeal:0,clears:[],firstClears:[],shop:{},day:d,runs:0};if(state.event.day!==d){state.event.day=d;state.event.runs=0;}if((Number(state.event.runs)||0)>=EVENT_DAILY_ATTEMPTS)throw codeError('EVENT_ATTEMPTS_EXHAUSTED');state.event.runs=(Number(state.event.runs)||0)+1;rewardState(state,st.reward);const first=!state.event.firstClears.includes(id);if(first){state.event.firstClears.push(id);rewardState(state,st.first||{});}if(!state.event.clears.includes(id))state.event.clears.push(id);recordCombatProgress(state,{battles:1,wins:1});state.updatedAt=nowIso();return{id,name:st.name,reward:st.reward,firstReward:first?st.first:null,first};}
function eventShopBuy(state,id){const item=EVENT_SHOP.find(x=>x.id===id);if(!item)throw codeError('EVENT_SHOP_ITEM_NOT_FOUND');ensureEventShop(state);const bought=Number(state.event.shop[id])||0;if(bought>=item.limit)throw codeError('EVENT_SHOP_LIMIT_REACHED');if((Number(state.event.moonSeal)||0)<item.cost)throw codeError('NOT_ENOUGH_MOON_SEAL');state.event.moonSeal-=item.cost;const fragmentResult=item.reward.heroId?grantHeroFragments(state,item.reward.heroId,item.reward.fragments||0):null;if(!fragmentResult)rewardState(state,item.reward);state.event.shop[id]=bought+1;state.updatedAt=nowIso();return{id,reward:item.reward,bought:bought+1,left:item.limit-bought-1,fragmentBanked:!!fragmentResult?.pending};}

function ensureResourceRuns(state){const day=observedDay(state);if(!state.resourceRuns||state.resourceRuns.day!==day)state.resourceRuns={day,used:{}};state.resourceRuns.used||={};return state.resourceRuns;}
function resourceDungeonById(id){return RESOURCE_DUNGEONS.find(x=>x.id===id);}
function resourceDungeon(state,id){ensureResourceRuns(state);const d=resourceDungeonById(id);if(!d)throw codeError('DUNGEON_NOT_FOUND');const used=Number(state.resourceRuns.used[id])||0;if(used>=d.attempts)throw codeError('DUNGEON_ATTEMPTS_EXHAUSTED');if(formationPower(state)<d.power)throw codeError('POWER_TOO_LOW');state.resourceRuns.used[id]=used+1;rewardState(state,d.reward);recordCombatProgress(state,{battles:1,wins:1,resource:1});state.updatedAt=nowIso();return{id,name:d.name,reward:d.reward,used:used+1,left:d.attempts-used-1};}
function achievementProgress(state,a){if(a.type==='heroCount')return Object.keys(state.heroes).length;if(a.type==='ssrCount')return Object.keys(state.heroes).filter(id=>byId(id)?.rarity==='SSR').length;if(a.type==='stage')return state.campaignCompleted?30:state.stage;if(a.type==='bossCount')return state.bossClears.length;if(a.type==='maxEnhance')return Math.max(0,...Object.values(state.inventory?.enhance||{}).map(Number).filter(Number.isFinite));if(a.type==='power')return formationPower(state);if(a.type==='craftedCount')return Number(state.forge?.crafted)||0;if(a.type==='maxBondLevel')return Math.max(1,...Object.keys(state.heroes).map(id=>bondInfo(state,id).level));if(a.type==='eventClears')return state.event?.clears?.length||0;return 0;}
function claimAchievement(state,id){const a=ACHIEVEMENTS.find(x=>x.id===id);if(!a)throw codeError('ACHIEVEMENT_NOT_FOUND');if(state.achievements?.claimed?.[id])throw codeError('ACHIEVEMENT_ALREADY_CLAIMED');const p=achievementProgress(state,a);if(!Number.isFinite(p)||p<a.goal)throw codeError('ACHIEVEMENT_NOT_READY');const reward={...a.reward};if(reward.signatureWeapon&&(Number(state.inventory?.signatureWeapons?.[reward.signatureWeapon])||0)>=1){const item=signatureById(reward.signatureWeapon);delete reward.signatureWeapon;reward.forgeDust=(Number(reward.forgeDust)||0)+(item?.rarity==='SSR'?300:item?.rarity==='SR'?180:100);}rewardState(state,reward);state.achievements.claimed[id]=true;state.updatedAt=nowIso();return{id,reward,progress:p};}
...
function enemyTeam(stage){
  ...
  if(boss){const minionScale=1+(stage-1)*.09;const minions=Array.from({length:2},(_,i)=>({id:`m${i}`,name:`裂境侍从 ${stage}-${i+1}`,hp:Math.round((600+stage*55)*minionScale),atk:Math.round((65+stage*6)*minionScale),def:Math.round((30+stage*2.8)*minionScale),spd:84+i*5+Math.floor(stage/2),skill:'裂境冲击',skillType:'enemy'}));return[{...boss,id:`boss-${stage}`,isBoss:true},...minions];}
  const size=Math.min(5,2+Math.floor((stage-1)/3)),scale=1+(stage-1)*.055;
  return Array.from({length:size},(_,i)=>({id:`e${i}`,name:`裂境体 ${stage}-${i+1}`,hp:Math.round((760+stage*65)*scale),atk:Math.round((88+stage*8.5)*scale),def:Math.round((44+stage*3.6)*scale),spd:80+i*4+stage,skill:'裂境冲击',skillType:'enemy'}));
}
...
function skillAction(actor,allies,enemies,rng,log,targetId=null){
  ...
  switch(actor.skillType){
    ...
    case'cleanseSupport':{let total=0,clean=0;for(const t of friends){const actual=Math.min(Math.round(actor.atk*.45*(1+(actor.signatureBonus?.healPct||0)+(actor.talentBonus?.healPct||0))),t.maxHp-t.hp);t.hp+=actual;t.atkBuff=Math.max(t.atkBuff,2);if(t.stun||t.silence||t.slow){clean++;t.stun=0;t.silence=0;t.slow=0;}total+=actual;}log.push(`${actor.name}施放「${actor.skill}」，恢复全队 ${total} 生命、强化攻击并净化 ${clean} 个异常状态`);break;}
    case'buff':{for(const t of friends)t.atkBuff=Math.max(t.atkBuff,2);log.push(`${actor.name}点亮「${actor.skill}」，全队攻击提升2回合`);break;}
    case'stun':{const t=targetRandom(),d=hit(t,1.1),ok=rng()<Math.min(.9,.38+(actor.talentBonus?.effectPct||0));if(ok)t.stun=Math.max(t.stun,1);log.push(`${actor.name}施放「${actor.skill}」，造成 ${d.hpDamage} 伤害${mark(d)}${ok?'并眩晕目标':''}`);break;}
    case'silence':{const t=targetRandom(),d=hit(t,1.05),duration=1+(rng()<(actor.talentBonus?.effectPct||0)?1:0);t.slow=Math.max(t.slow,1);t.silence=Math.max(t.silence,duration);log.push(`${actor.name}施放「${actor.skill}」，造成 ${d.hpDamage} 伤害${mark(d)}并使 ${t.name} 沉默${duration}回合`);break;}
...
function bossSkillAction(actor,allies,enemies,rng,log){const foes=living(enemies);if(!foes.length)return;const phaseCfg=BOSS_PHASE_SKILLS[actor.stage]?.[actor.phase];if(phaseCfg){if(phaseCfg.shieldPct)actor.shield+=Math.round(actor.maxHp*phaseCfg.shieldPct);if(phaseCfg.target==='lowest'){const t=[...foes].sort((a,b)=>a.hp/a.maxHp-b.hp/b.maxHp)[0],d=applyDamage(t,actor.atk*phaseCfg.mult-t.def*.32,actor);log.push(`⚠ ${actor.name}发动阶段技能「${phaseCfg.name}」，对 ${t.name} 造成 ${d.hpDamage} 伤害`);return;}let total=0,affected=0;for(const t of foes){total+=applyDamage(t,actor.atk*phaseCfg.mult-t.def*.30,actor).hpDamage;if(phaseCfg.status&&rng()<phaseCfg.chance){if(phaseCfg.status==='stun')t.stun=Math.max(t.stun,1);if(phaseCfg.status==='silence')t.silence=Math.max(t.silence,1);if(phaseCfg.status==='slow')t.slow=Math.max(t.slow,2);affected++;}}log.push(`⚠ ${actor.name}发动阶段技能「${phaseCfg.name}」，造成 ${total} 群体伤害${phaseCfg.shieldPct?'并获得阶段护盾':''}${affected?`，附加${phaseCfg.status==='silence'?'沉默':phaseCfg.status==='stun'?'眩晕':'减速'} ${affected} 人`:''}`);return;}switch(actor.skillType){
  ...
  default:{const t=randomPick(foes,rng),d=applyDamage(t,actor.atk*1.45-t.def*.5,actor);log.push(`${actor.name}施放「${actor.skill}」，造成 ${d.hpDamage} 伤害`);}
}}
...
function triggerCombo(allies,enemies,skillCasts,used,stats,log){const formation=allies.map(x=>x.id);let c;while((c=comboForCast(skillCasts,used,formation))){used.push(c.id);const members=c.members.map(id=>allies.find(x=>x.id===id)).filter(Boolean);if(members.length<2)break;if(c.id==='mirrorTide'){let heal=0,shield=0;for(const t of living(allies)){const h=Math.min(Math.round(t.maxHp*.12),t.maxHp-t.hp);t.hp+=h;const sh=Math.round(t.maxHp*.08);t.shield+=sh;heal+=h;shield+=sh;}for(const m of members){const st=ensureStat(stats,m.id,m.name);st.heal+=Math.round(heal/members.length);st.shield+=Math.round(shield/members.length);st.combo++;}log.push(`✦ 连携技「${c.name}」发动：全队恢复 ${heal} 生命并获得 ${shield} 护盾`);}else{const power=members.reduce((n,x)=>n+x.atk,0),mult=c.id==='starSever'?.78:.68;let total=0;for(const t of living(enemies)){const d=applyDamage(t,power*mult-t.def*.22);total+=d.hpDamage;applyBossBreak(t,c.id==='starSever'?28:18);}for(const m of members){const st=ensureStat(stats,m.id,m.name);st.damage+=Math.round(total/members.length);st.combo++;}log.push(`✦ 连携技「${c.name}」发动，对敌方全体造成 ${total} 伤害`);flushBreakNotices(enemies,log);}}}
...
function battle(state,rng=rand01,options={}){
  ...
  if(state.campaignCompleted)throw codeError('CAMPAIGN_COMPLETED');if(!state.formation.length)throw codeError('INVALID_FORMATION');const oldSession=state.activeBattleSession;if(oldSession&&['running','waiting'].includes(oldSession.status))throw codeError('ACTIVE_BATTLE_SESSION');const skillMode=options.skillMode==='manual'?'manual':'auto',manualSkills=[...new Set((options.manualSkills||[]).filter(id=>state.formation.includes(id)))].slice(0,3),autoStrategy=AUTO_STRATEGIES[options.autoStrategy]?options.autoStrategy:(state.battlePrefs?.autoStrategy||'balanced');state.battlePrefs={skillMode,manualSkills,autoStrategy};const stage=state.stage,story=storyForStage(stage);state.storySeen||=[];if(story&&!state.storySeen.includes(story.id))state.storySeen.push(story.id);const boss=bossByStage(stage),allies=state.formation.map((id,i)=>makeUnit(byId(id),state.heroes[id],state,state.formationRows?.[id]||(i<2?'front':'back')));
  ...
  while(living(allies).length&&living(enemies).length&&round<36){round++;log.push(`— 第 ${round} 回合 —`);const actors=[...allies.map(u=>({side:'a',u})),...enemies.map(u=>({side:'e',u}))].filter(x=>x.u.hp>0).sort((a,b)=>effectiveSpeed(b.u)-effectiveSpeed(a.u));
    for(const a of actors){if(a.u.hp<=0)continue;if(a.u.stun>0){a.u.stun--;if(a.u.breakWindow>0)a.u.breakWindow--;const text=`${a.u.name}处于眩晕，本次无法行动`;log.push(text);snapshot(round,text,a.u,a.side);continue;}const foes=a.side==='a'?enemies:allies,friends=a.side==='a'?allies:enemies;if(!living(foes).length)break;a.u.turns++;if(a.u.cooldownLeft>0)a.u.cooldownLeft--;a.u.energy=Math.min(120,(a.u.energy||0)+(a.u.energyGain||34));refreshActor(a.u);const before=log.length,metricBefore=metricSnapshot(friends,foes);let allySkillCast=false;const skillReady=a.u.energy>=(a.u.skillCost||100)&&a.u.cooldownLeft<=0,canSkill=skillReady&&(a.u.silence||0)<=0,useSkill=canSkill&&(a.side!=='a'||skillMode==='auto'||manualSkills.includes(a.u.id));if(skillReady&&!canSkill){const text=`${a.u.name}处于沉默，无法释放技能`;log.push(text);snapshot(round,text,a.u,a.side);basicAttack(a.u,foes,rng,log);a.u.energy=Math.min(a.u.energy,Math.max(0,(a.u.skillCost||100)-20));}else if(skillReady&&a.side==='a'&&skillMode==='manual'&&!manualSkills.includes(a.u.id)){const text=`${a.u.name}技能就绪，但手动策略未授权释放`;log.push(text);snapshot(round,text,a.u,a.side);basicAttack(a.u,foes,rng,log);}else if(useSkill&&a.side==='a'){if(shouldAutoUseSkill(a.u,friends,foes,autoStrategy)){skillAction(a.u,friends,foes,rng,log,autoTargetFor(a.u,friends,foes,autoStrategy));a.u.energy=Math.max(0,a.u.energy-(a.u.skillCost||100));a.u.cooldownLeft=a.u.skillCooldown||0;skillCasts[a.u.id]=(skillCasts[a.u.id]||0)+1;ensureStat(stats,a.u.id,a.u.name).skills++;allySkillCast=true;}else basicAttack(a.u,foes,rng,log);}else if(useSkill&&a.u.isBoss){bossSkillAction(a.u,friends,foes,rng,log);a.u.energy=Math.max(0,a.u.energy-(a.u.skillCost||100));a.u.cooldownLeft=a.u.skillCooldown||0;}else if(useSkill){const t=preferredTarget(foes,rng,null,a.u),d=applyDamage(t,a.u.atk*1.45-t.def*.5+(rng()*12-6),a.u);log.push(`${a.u.name}施放「${a.u.skill}」，对 ${t.name} 造成 ${d.hpDamage} 伤害`);a.u.energy=Math.max(0,a.u.energy-(a.u.skillCost||100));a.u.cooldownLeft=a.u.skillCooldown||0;}else basicAttack(a.u,foes,rng,log);if(a.side==='a')recordMetrics(stats,a.u,metricBefore,friends,foes);if(allySkillCast)triggerCombo(allies,enemies,skillCasts,combosUsed,stats,log);const text=log[log.length-1]||'';if(log.length>before)snapshot(round,text,a.u,a.side);const bossUnit=enemies.find(x=>x.isBoss&&x.hp>0),phaseText=updateBossPhaseUnit(bossUnit,stage);if(phaseText){log.push(phaseText);snapshot(round,phaseText,bossUnit,'e');}endActorTurn(a.u);}
  }
  const win=living(allies).length>0&&living(enemies).length===0;let rewards={coin:0,tickets:0},drops=[];recordCombatProgress(state,{battles:1,wins:win?1:0});
  if(win){rewards={coin:350+stage*40+(boss?500:0),tickets:stage%5===0?2:1};rewardState(state,rewards);drops=rollLoot(state,stage,rng,!!boss);if(stage<30)state.stage++;else state.campaignCompleted=true;log.push(`胜利！获得烬币 ${rewards.coin}、契灵印 ${rewards.tickets}`);for(const d of drops)log.push(d.converted?.forgeDust?`✦ 重复遗物「${d.item.name}」自动转化为锻造尘 ${d.converted.forgeDust}`:`✦ 获得${d.kind==='relic'?'遗物':'装备'}「${d.item.name}」${d.item.rarity}`);}
...
function isPristine(state){const s=migrate(state),h=s.heroes?.h004,inv=s.inventory||{},sum=o=>Object.values(o||{}).reduce((n,v)=>n+(Number(v)||0),0),anyClaim=o=>Object.values(o||{}).some(Boolean);return s.stage===1&&!s.campaignCompleted&&Object.keys(s.heroes||{}).length===1&&!!h&&safeInt(h.level,1,1)===1&&safeInt(h.star,1,1)===1&&safeInt(h.awakening,0,0)===0&&safeInt(h.fragments,0,0)===0&&!h.signature&&s.coin===5000&&s.tickets===30&&s.starCrystal===300&&s.gachaHistory.length===0&&!s.newbieGacha?.claimed&&(s.storySeen||[]).length===0&&!s.activeBattleSession&&!Object.values(s.talents||{}).some(x=>Array.isArray(x)&&x.length)&&safeInt(s.bond?.h004?.points,0,0)===0&&Object.keys(inv.equipment||{}).length===1&&safeInt(inv.equipment?.eq001,0,0)===1&&sum(inv.relics)===0&&sum(inv.signatureWeapons)===0&&sum(inv.heroFragments)===0&&safeInt(inv.starIron,0,0)===120&&safeInt(inv.forgeDust,0,0)===0&&safeInt(inv.bondGift,0,0)===3&&safeInt(inv.awakeningCore,0,0)===0&&safeInt(inv.talentPoints,0,0)===3&&safeInt(inv.enhance?.eq001,0,0)===0&&sum(s.resourceRuns?.used)===0&&safeInt(s.sweep?.used,0,0)===0&&safeInt(s.event?.runs,0,0)===0&&safeInt(s.event?.moonSeal,0,0)===0&&(s.event?.clears||[]).length===0&&safeInt(s.nightEvent?.runs,0,0)===0&&safeInt(s.nightEvent?.voyageBadge,0,0)===0&&(s.nightEvent?.clears||[]).length===0&&safeInt(s.eventBoss?.runs,0,0)===0&&safeInt(s.eventBoss?.totalDamage,0,0)===0&&safeInt(s.forge?.crafted,0,0)===0&&safeInt(s.forge?.dismantled,0,0)===0&&!anyClaim(s.daily?.claimed)&&!anyClaim(s.weekly?.claimed)&&!anyClaim(s.achievements?.claimed)&&safeInt(s.loginReward?.totalClaims,0,0)===0&&!(s.mails||[]).some(m=>m.claimed);}
module.exports={HEROES,EQUIPMENT,RELICS,BOSSES,BOSS_PHASES,BOSS_PHASE_SKILLS,AUTO_STRATEGIES,SKILL_RULES,COMBO_SKILLS,BOSS_BREAK_RULES,ROLE_CLASS,COMBAT_CLASSES,CLASS_ADVANTAGE,CLASS_DISADVANTAGE,TALENT_TREES,TALENT_COSTS,SWEEP_DAILY_LIMIT,NIGHT_EVENT_CONFIG,NIGHT_EVENT_STAGES,NIGHT_EVENT_SHOP,CHAPTERS,FACTION_BONUSES,RESOURCE_DUNGEONS,ACHIEVEMENTS,HERO_VISUALS,EQUIPMENT_SETS,FORGE_RECIPES,DISMANTLE_DUST,EVENT_CONFIG,EVENT_DAILY_ATTEMPTS,EVENT_STAGES,EVENT_SHOP,SIGNATURE_WEAPONS,AWAKEN_COST,SYSTEM_MAILS,EVENT_BOSS,BOND_THRESHOLDS,BOND_STORIES,HERO_ARCHIVES,STORY_NODES,HERO_VOICES,SKILL_FX,CONFIG_VERSION,SAVE_VERSION,FRAGMENTS_PER_DUP,STAR_COST,DAILY_TASKS,DAILY_ALL_REWARD,LOGIN_REWARDS,WEEKLY_TASKS,WEEKLY_ALL_REWARD,SHOP_ITEMS,LIMITED_POOL,byId,equipmentById,relicById,signatureById,signatureForHero,bossByStage,chapterByStage,resourceDungeonById,eventStageById,storyForStage,archiveForHero,newPlayer,migrate,ensureDaily,ensureWeekly,ensureShop,ensureEventShop,ensureNightEventShop,ensureEventBoss,ensureResourceRuns,dailyProgress,weeklyProgress,recordCombatProgress,pendingHeroFragments,grantHeroFragments,claimDailyTask,claimDailyAll,loginRewardInfo,claimLoginReward,claimWeeklyTask,claimWeeklyAll,shopBuy,pull,pullStarterTen,pullLimited,levelUp,starUp,setFormation,saveFormationPreset,loadFormationPreset,sweepInfo,sweepStage,talentTree,heroTalents,talentBonus,unlockTalent,combatClassForHero,combatClassInfo,classMultiplier,nightEventStageById,nightEventBattle,nightEventShopBuy,equipGear,unequipGear,equipRelic,enhanceCost,enhanceEquipment,dismantleEquipment,forgeRecipeById,forgeEquipment,equipSignature,awakenInfo,awakenHero,unreadMailCount,claimMail,claimAllMail,bondInfo,giftHero,eventBattle,eventShopBuy,eventBossBattle,localEventLeaderboard,resourceDungeon,achievementProgress,claimAchievement,equipmentSetCounts,equipmentSetBonus,activeEquipmentSets,equipmentStats,factionCounts,activeFactionBonuses,computedStats,heroPower,formationPower,enemyTeam,bossPhaseInfo,updateBossPhaseUnit,skillTargetMode,autoStrategyInfo,battleStatRows,battleReplays,battle,battleSessionView,startBattleSession,battleSessionCommand,resumeBattleSession,cancelBattleSession,advanceBattleSession,codeError,publicState,nowIso,gameDay,isPristine,uuid:()=>crypto.randomBytes(8).toString('hex')};
